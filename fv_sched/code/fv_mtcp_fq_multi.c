...
static in_port_t dport_normal[MAX_FNUM];
...
void RunSendContext(void *arg)
{
    ...
    int ep, core, ret;
    int i = 0, j = 0, k = 0;
	int idx, end_num = 0, flag = False, per_port_num, per_flow_num;
    int complete_conn = 0;
    thread_context_t ctx;
    struct mtcp_epoll_event *events;
    mctx_t mctx;
    char buf[BUF_SIZE];
    struct timeval cur_tv, prev_tv, start_tv;
    uint64_t diff_ts, snd_bytes = 0;
    uint32_t start_ts[PRT_PF] = { 0, 10, 20, 30 }, end_ts[PRT_PF] = { 40, 50, 60, 70 };
    int ifstart[PRT_PF] = { False }, ifend[PRT_PF] = { False };
    ...
    per_port_num = MAX_FNUM / PRT_PF;
    int evid[per_port_num] = { 0 };
    // assert(connection < per_flow_num);
    per_flow_num = per_port_num / PRT_PF;
    idx = core * per_port_num;
    for (i = 0; i < per_port_num; i++) {
        mtcp_init_rss(mctx, saddr, 1, daddr, dport_normal[idx+i]);
    }
    ...
    while (!done[core]) {
        ...
        for (j = 0; j < PRT_PF; j++) {
            gettimeofday(&cur_tv, NULL);
            diff_ts = TIMEVAL_TO_MSEC(cur_tv) - TIMEVAL_TO_MSEC(start_tv);
            if (!ifstart[j] && diff_ts >= start_ts[j]*test_interval) {
                ifstart[j] = True;
                for (k = 0; k < connection; k++) {
                    CreateConnection(mctx, core, ep, NORMAL+idx+j*per_port_num+k);
                }
            }
            else if (!ifend[j] && diff_ts >= end_ts[j]*test_interval) {
                ifend[j] = True;
                for (k = 0; k < connection; k++) {
                    mtcp_epoll_ctl(mctx, ep, MTCP_EPOLL_CTL_DEL, evid[j*per_flow_num+k], 0);
                    mtcp_close(mctx, evid[j*per_flow_num+k]);
                }
                end_num++;
                TRACE_CONFIG("[CPU %d] end num %d.\n", ctx->core, end_num);
                if (end_num == PRT_PF) {
                    flag = True;
                }
            }
        ...
        if (nevents > 0) {
            for (i = 0; i < nevents; i++) {
                if (events[i].events == MTCP_EPOLLOUT) {
                    for (j = 0; j < PRT_PF; j++) {
                        for (k = 0; k < connection; k++) {
                            if (evid[j*per_flow_num+k] == events[i].data.sockid) {
                                break;
                            }
                            else if (evid[j*per_flow_num+k] == 0) {
                                evid[j*per_flow_num+k] = events[i].data.sockid;
                                break;
                            }
                        }
                    }
...
void RunRecvContext(void *arg)
{
    ...
    uint64_t diff_ts, recv_bytes[PRT_PF] = { 0 };
    ...
    per_port_num = MAX_FNUM / PRT_PF;
    int evid[per_port_num] = { 0 }, listen[per_port_num] = { 0 };
    // assert(connection < per_flow_num);
    per_flow_num = per_port_num / PRT_PF;
    idx = (core - PRT_PF) * per_port_num;
    for (i = 0; i < per_port_num; i++) {
        listen[i] = ServicePortConfig(dport_normal[idx+i], mctx, ep);
    }
    ...
    while (!done[core]) {
		...
		for (i = 0; i < nevents; i++) {
			...
			if (events[i].events == MTCP_EPOLLIN) {
                ...
                if (ev_sockid == listen[0] || ev_sockid == listen[1] || ev_sockid == listen[2] || ev_sockid == listen[3]) {
                    ...
                    if (diff_ts > log_interval) {
                        cnt++;
                        for (j = 0; j < PRT_PF; j++) {
                            fprintf(stderr, "[CPU %d] fq %d flow %d %.2f Gbps\n", ctx->core, cnt, ntohs(dport_normal[idx+j]), recv_bytes[j]*8.0/(diff_ts*1000000));
                            recv_bytes[j] = 0;
                        }
                        prev_tv = cur_tv;
                    }
...
int main(int argc, char **argv)
{
	...
	for (i = 2; i < argc; i++) {
		if (strcmp(argv[i], "-p") == 0) {
			connection = mystrtol(argv[i+1], 10);
		}
