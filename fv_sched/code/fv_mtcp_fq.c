...
static in_port_t dport_normal[FNUM];
...
void RunSendContext(void *arg)
{
    ...
    int ep, core, ret;
	int i = 0, j, idx, end_num = 0, evid[PRT_PF] = { 0 }, flag = False;
    int complete_conn = 0;
    thread_context_t ctx;
    ...
    uint32_t start_ts[PRT_PF] = { 0, 20, 40, 60 }, end_ts[PRT_PF] = { 80, 80, 80, 100 };
    ...
    while (!done[core]) {
        ...
        for (j = 0; j < PRT_PF; j++) {
            ...
            if (!ifstart[j] && diff_ts >= start_ts[j]*test_interval) {
                ifstart[j] = True;
                CreateConnection(mctx, core, ep, NORMAL+idx+j);
            }
            else if (!ifend[j] && diff_ts >= end_ts[j]*test_interval) {
                ifend[j] = True;
                mtcp_epoll_ctl(mctx, ep, MTCP_EPOLL_CTL_DEL, evid[j], 0);
                mtcp_close(mctx, evid[j]);
                end_num++;
        ...
        if (nevents > 0) {
            for (i = 0; i < nevents; i++) {
                if (events[i].events == MTCP_EPOLLOUT) {
                    for (j = 0; j < PRT_PF; j++) {
                        if (evid[j] == events[i].data.sockid) {
                            break;
                        }
                        else if (evid[j] == 0) {
                            evid[j] = events[i].data.sockid;
                            break;
                        }
                    }
...
void RunRecvContext(void *arg)
{
    ...
    int listen[PRT_PF] = { 0 }, evid[PRT_PF] = { 0 };
    int i, j, idx, cnt = 0;
